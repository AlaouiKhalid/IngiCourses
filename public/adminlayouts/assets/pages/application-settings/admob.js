...
$('form#editAdMob input').change(function() {
	isChanged = true;
});

    var request;
    //edit form
	$('#editAdMob').submit(function(event) {
		...
		$('.saveBtn').val('Saving AdMob settings ...');
		...
		request.done(function(response, textStatus, jqXHR) {
			if (response.status == 'true') {
				Swal.fire({
					title: 'AdMob settings updated successfully!',
					text: 'New AdMob settings has been updated!',
					type: 'success'
                }).then((result) => {
                    isChanged = false;
                });				
