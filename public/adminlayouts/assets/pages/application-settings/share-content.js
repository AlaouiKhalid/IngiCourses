...
$('form#share-content textarea').change(function() {
	isChanged = true;
});

    var request;
    //edit form
	$('#share-content').submit(function(event) {
		...
		$('.saveBtn').val('Saving new share content ...');
		...
		request.done(function(response, textStatus, jqXHR) {
			if (response.status == 'true') {
				Swal.fire({
					...
					text: 'New share content has been updated!',
					type: 'success'
                }).then((result) => {
                    isChanged = false;
                });				
